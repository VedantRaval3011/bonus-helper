    ...
    try {
      ...
      if (bonusWorkbook.SheetNames.length > 1) {
        ...
        for (let rowIdx = 0; rowIdx < staffData.length; rowIdx++) {
          ...
          if (hasEmpCode) {
            ...
            const actualIdx = headers.findIndex((h: any) =>
              /^ACTUAL$/i.test(String(h ?? "").trim())
            );

            if (empCodeIdx === -1 || actualIdx === -1) continue;
            ...
            for (let i = rowIdx + 1; i < staffData.length; i++) {
              ...
              const actualHR = Number(dataRow[actualIdx]) || 0;
              ...
              const empData = hrActualDataMap.get(empCode)!;
              empData.actualHRValues.push(actualHR);
            }
          }
        }
      }

      console.log(`\n✅ HR Data loaded: ${hrActualDataMap.size} employees`);
      ...
      for (const [empId, softwareData] of softwareEmployeesData) {
        ...
        comparison.push({
          ...
          actualHR: actualHR,
          actualHRCount: hrData?.actualHRValues.length || 0,
