    ...
    try {
        ...
        if (perSheet) {
          ...
          for (let i = 1; i < perData.length; i++) {
            ...
            if (empCode && !isNaN(empCode) && bonusPercentage && !isNaN(bonusPercentage)) {
              ...
              console.log(`   Emp ${empCode}: Custom percentage for ACTUAL = ${bonusPercentage}%`);
            }
          }
        }
        ...
        if (avgSheet) {
          ...
          for (let i = 1; i < avgData.length; i++) {
            ...
            if (empCode && !isNaN(empCode)) {
              ...
              console.log(`   Emp ${empCode}: October excluded from GROSS & Register`);
            }
          }
        }
      ...
      for (const [empId, rec] of staffEmployees) {
        ...
        if (hasSep2025 && !zeroOctoberEmployees.has(empId)) {
          ...
          estOct = 0;
          console.log(`🔴 Emp ${empId}: October excluded from GROSS & Register`);
        }
      ...
      for (const [empId, softwareData] of softwareEmployeesData) {
        ...
        if (customPercentageMap.has(empId)) {
          ...
          console.log(`🎯 Emp ${empId}: Using custom percentage for ACTUAL = ${percentageForActual}%`);
        } else {
        ...
        console.log(
          `✅ Emp ${empId}${octNote}${customNote}: GROSS=₹${softwareData.grossSal.toFixed(2)} | ` +
          `Register(8.33%)=₹${registerCalculated.toFixed(2)} - Actual(${percentageForActual}%)=₹${actualCalculated.toFixed(2)} = ` +
          `Reim(Calc)=₹${reimCalculated.toFixed(2)} | Reim(HR)=₹${reimHR.toFixed(2)} | ${status}`
        );
      }
    ...
    >
      {file ? (
        ...
        <div className="bg-white rounded-lg p-4 border border-gray-200">
          ...
          <p className="text-xs text-gray-500">{optional ? "Will use calculated percentages" : "Upload in Step 1"}</p>
        </div>
      )}
    </div>
  );
          ...
          <div className="flex justify-between items-center mb-8">
            <div>
              ...
              <p className="text-gray-600 mt-2">
                Formula: Reimbursement = Register - Actual (with custom percentages)
              </p>
            </div>
          ...
          <div className="mb-8 bg-blue-50 border border-blue-200 rounded-lg p-6">
            ...
            <div className="text-sm text-blue-800 space-y-3">
              <div>
                ...
                <p className="ml-4 text-lg"><strong>Reimbursement = Register - Actual</strong></p>
              </div>
              
              <div>
                <p className="font-semibold mb-1">Calculation Steps:</p>
                <ul className="list-disc ml-8 space-y-1">
                  <li><strong>GROSS SAL:</strong> Sum of monthly SALARY1 + October estimate</li>
                  <li><strong>Register:</strong> GROSS SAL × 8.33% (fixed for all employees)</li>
                  <li><strong>Actual:</strong> Calculated using custom or DOJ-based percentage</li>
                  <li><strong>Reimbursement:</strong> Register - Actual</li>
                </ul>
              </div>

              <div className="bg-purple-50 border border-purple-200 rounded p-3 mt-3">
                <p className="font-semibold text-purple-900 mb-1">🎯 Custom Percentage (ACTUAL only):</p>
              ...
              <div className="bg-orange-50 border border-orange-200 rounded p-3">
                <p className="font-semibold text-orange-900 mb-1">🔴 October Exclusion Rule:</p>
