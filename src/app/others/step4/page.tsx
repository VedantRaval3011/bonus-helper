...
const calculatePercentage = (dateOfJoining: any): number => {
  if (!dateOfJoining) {
    console.log(`⚠️ No DOJ provided, returning 0%`);
    return 0;
  }
  ...
  if (typeof dateOfJoining === "number") {
    ...
    if (ddmmyyMatch) {
      ...
      console.log(`✅ Parsed dd.mm.yy format: ${dateOfJoining} → ${doj.toLocaleDateString()}`);
    } else {
  ...
  if (isNaN(doj.getTime())) {
    console.log(`⚠️ Invalid DOJ format: ${dateOfJoining} (type: ${typeof dateOfJoining}), returning 0%`);
    return 0;
  }

  const referenceDate = new Date(2025, 9, 30); // October 30, 2025
...
// Use Sheet1 which contains both Worker and Staff data
const staffSheetName = "Sheet1";

if (!bonusWorkbook.SheetNames.includes(staffSheetName)) {
  console.error("Sheet1 not found in bonus file!");
  console.log("Available sheets:", bonusWorkbook.SheetNames);
  setError(`Sheet1 not found. Available sheets: ${bonusWorkbook.SheetNames.join(", ")}`);
  setIsProcessing(false);
  return;
}

console.log(`Processing bonus sheet: ${staffSheetName} (filtering for Staff only)`);
...
const deptIdx = headers.findIndex((h: any) => {
  const hStr = String(h || "").toUpperCase();
  return hStr.includes("DEPT") || hStr === "DEPTT." || hStr === "DEPTT" || hStr.includes("DEPARTMENT");
});
...
const dept = deptIdx !== -1 ? String(row[deptIdx] || "").trim().toUpperCase() : "STAFF";

if (!empId || isNaN(empId)) continue;

// Only process STAFF employees (skip WORKER - check for W, WORKER, WORKERS)
if (dept === "WORKER" || dept === "WORKERS" || dept === "W") {
  console.log(`Skipping Employee ${empId} (${empName}): Department is ${dept} (Worker)`);
  continue;
}
...
console.log(`Sheet1: Processed ${processedCount} unique STAFF employees (Workers filtered out)`);
      console.log(
          ...
          <div className="flex justify-between items-center mb-8">
            <div>
              <h1 className="text-3xl font-bold text-gray-800">
                Step 4 - Staff Gross2 Calculation
              </h1>
            </div>
